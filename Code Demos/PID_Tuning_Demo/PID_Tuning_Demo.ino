...
void driveStraight(int mmDistance, int mmVelocity, float kProp, float kInt, float kDer){
  ...
  nCount=startPID(mmDistance,mmVelocity,kProp,kInt,kDer); // function that sets distance in mm and PWM intensity, (float mm to drive, float mm/s drive, kPID terms)
...
int startPID(float distance, int velNominal, float kProportional, float kIntegral, float kDerivative){
  //update values
  lErrIntegral=0;
  rErrIntegral=0;
  velNom=velNominal; //mm/s
