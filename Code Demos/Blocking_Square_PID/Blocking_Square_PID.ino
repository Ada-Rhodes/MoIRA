...
void driveStraight(int mmDistance, int mmVelocity, float kProp, float kInt, float kDer){
  // start driving
  encoder.clearEnc(BOTH);  // Reset the counters.
  nCount=startPID(mmDistance,mmVelocity,kProp,kInt,kDer); // function that sets distance in mm and PWM intensity

  // loop through functions while distance not reached
  while ((lCount <= (nCount)) && (rCount <= (nCount))){
    // non-blocking functions go here 
    encoderCount(); // 2nd update encoder count
    pidForward(driveState, pwmNominal, 1, 1); // 3rd drive motors 
    report(); // 4th print out values for debug 
    checkGoal(); // 5th determine if goal position reached 
  }
}

void driveTurnTight(int radianTurn, int mmVelocity, float kProp, float kInt, float kDer){
  // start driving
  encoder.clearEnc(BOTH);  // Reset the counters.
  
  nCount=startPID((radianTurn*bodyW)/2,mmVelocity,kProp,kInt,kDer); // function that sets distance in mm and PWM intensity
...
void driveTurnWide(int radianTurn, int mmVelocity, float kProp, float kInt, float kDer){
  // start driving
  encoder.clearEnc(BOTH);  // Reset the counters.
  
  nCount=startPID((radianTurn*bodyW),mmVelocity,kProp,kInt,kDer); // function that sets distance in mm and PWM intensity
  ...
  while (driveState==1){
    ...
    encoderCount(); // 2nd update encoder count
    rCount=-1*rCount;
    pidForward(driveState, pwmNominal,1,0); // 3rd drive motors 
    report(); // 4th print out values for debug 
    checkGoal(); // 5th determine if goal position reached 
  }
}
...
void pidForward(int driveSt, int pwnNom, int lDirection, int rDirection){ // 3rd
  if (driveSt==0){
    return;
  } else{
    //MATH
